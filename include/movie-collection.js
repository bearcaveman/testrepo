...
(function ($) {
	...
	function getYearStr($card) {
		...
		var ty = $.trim($card.find(".tile-year").first().text() || "");
		if (/^\d{4}$/.test(ty)) {
			return norm(ty);
		}
	...
	/** Visible tile shows one decimal; hidden .m-imdb keeps export precision for filters. */
	function roundImdbTileDisplay($root) {
		($root || $(document)).find(".movie-card .tile-imdb-val").each(function () {
			var n = parseFloat(String($(this).text()).replace(",", "."));
			if (!isNaN(n)) {
	...
	window.goRandom = function () {
		var pool = $(".movie-card").get();
		$(".movie-card").addClass(FILTER_HIDDEN);
		if (!pool.length) {
			$("#numMovies").text("0 Movies");
			try {
				$("#footer").trigger("updatelayout");
			} catch (e7) {}
			return false;
